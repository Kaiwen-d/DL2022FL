...
def get_transform(train, use_jitter=False):
    ...
    if train:
        if use_jitter:
            transforms.append(T.RandomPhotometricDistort(0.9))
            # transforms.append(T.ScaleJitter(target_size=(512,512)))
...
def main(args_multi_gpu):
    ...
    # init config
    config_path = os.path.abspath(f'./configs/{CONFIG}.yaml')
    ...
    if args.optim_mode == 'freeze':
        # freeze
        for p in model_without_ddp.backbone.backbone.parameters():
